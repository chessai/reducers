...
foldMapReduce1 :: (Semifoldable f, Reducer e m) => (a -> e) -> f a -> m
foldMapReduce1 f = semifoldMap (unit . f)
...
-- | Apply a 'Reducer' to a 'Semifoldable' mapping each element through 'unit'
foldReduce1 :: (Semifoldable f, Reducer e m) => f e -> m
foldReduce1 = semifoldMap unit
