...
instance Semifoldable (WithReducer m) where
  semifoldMap f = f . withoutReducer

instance Semitraversable (WithReducer m) where
  semitraverse f (WithReducer a) = WithReducer <$> f a
